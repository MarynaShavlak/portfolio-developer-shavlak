...
function createTechStackSection(stack) {
  ...
  Object.entries(stack).forEach(([key, items]) => {
    ...
    const $entry = $('<li class="tech-stack-wrapperlist-item"></li>').append(
      $cat,
      $itemList,
    );
    $list.append($entry);
...
function createFeaturesSection(features) {
  ...
  const $wrapper = $('<div class="project-features"></div>').append(
    '<p class="info-title">Features</p>',
  );
  const $list = $('<ul class="features-list"></ul>');

  features.forEach(({ title, items }, i) => {
    const $featureItem = $(
      `<li class="features-list-item" data-index="${i + 1}"></li>`,
    );
    const $featureTitle = $(`<p class="feature-category">${title}:</p>`);
    const $itemsList = $('<ul class="feature-items-list"></ul>');

    items.forEach((itm) =>
      $itemsList.append(`<li class="feature-item">${itm}</li>`),
    );
...
function createAdditionalInfoSection(info) {
  ...
  const $wrapper = $('<div class="project-additional-info"></div>').append(
    '<p class="info-title">Additional Info</p>',
  );
  const $list = $('<ul class="additional-info-list"></ul>');

  info.forEach(({ title, items }, i) => {
    const $section = $(
      `<li class="additional-info-item" data-index="${i + 1}"></li>`,
    );
    const $sectionTitle = $(
